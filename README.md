...
**新增**
- 搜索筛选：面板顶部搜索框，实时筛选按钮
- 收藏置顶：点击 ☆ 收藏按钮，永远排在最前面
- 面板透明度调节：头部滑块调整面板透明度
- Ctrl+M 快捷键：快速切换面板
- 首次使用引导弹窗：第一次打开面板时提示操作
- 菜单精简器全显/全隐按钮：每个分类一键全部显示或隐藏

**优化**
- 代码质量：var→let/const，DOM 选择器缓存，CSS 主题色适配
- 编辑按钮显示隐藏计数

### v1.1.2 (2025-06-04)

**变更**
- 精简器重排序只保留扩展菜单（extensionsSettings），移除了对左下菜单和魔棒的排序界面（这两个组的排序对 Magic Panel 无实际效果）
...
更新日志遵循 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.2.0/)。
