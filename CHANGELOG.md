...
- **自动发现缓存按需保存**：`refreshDiscoveryCache()` 只在发现缓存或 reorder 裁剪发生变化时写入设置，并向调用方返回真实 changed 状态，避免无变化重扫时重复 localStorage 写入、原生 DOM 重排和弹窗刷新。
- **默认设置独立实例化**：新增 `makeDefaultSettings()` / `normalizeSettings()`，读取失败、清除数据和首次初始化时都使用新的设置对象，避免共享嵌套对象带来的状态污染。
- **魔法面板热重载清理**：新增实例级监听器登记和 `destroy()`，热重载/重复注入时会清理旧面板和旧 document/window 监听器，按钮绑定也带实例 owner 标记。
...
- **主题系统**：4 套配色主题（琉璃/暖阳/暗紫/极光），通过弹窗表头 🎨 按钮循环切换，覆盖精简器面板 + 魔法面板。
- **CSS 变量架构**：18 个 `--mc-*` 变量控制弹窗外观，主题只需覆盖变量值，所有子元素自动联动。
- **魔法面板美化**：面板背景/边框/标题随主题变化，按钮背景/边框/图标/标签字色同步主题。
- **拖拽 Y 坐标降级**：`pointerup` 和 `touchend` 中当 `elementFromPoint` 返回自身或空时，按 Y 坐标找同一列的最近元素作为插入点。
- **导入/导出设置**：工具区新增 📤 导出设置 / 📥 导入设置按钮，支持跨设备迁移配置。
- **动画背景 + 辉光**：主题激活时弹窗背景缓慢流动渐变，active 标签呼吸辉光脉冲，拖拽条目霓虹辉光。

### Fixed

- **排序失效：reorder 数组累计过期选择器**：多次自动扫描后 `settings.reorder[]` 中的旧选择器未被清理，导致排序视图出现无法响应拖拽的死条目。`refreshDiscoveryCache` 末尾新增修剪步骤。
- **魔法面板 CSS 变量不生效**：`--mc-*` 变量设在 `.menu-cleaner-popup` 上，`.magic-panel` 为兄弟元素无法继承。`p()` 函数改为同时输出两端，`m()` 函数直接生成 `!important` 规则覆盖。
- **手机端无法在同一列内拖拽排序**：`pointerup` 缺少 Y 距离降级逻辑。
- **手机端拖拽残影**：`closePopup` 未清理触摸拖拽 ghost。
- **导入后主题不生效**：导入设置后未调用 `applyTheme()`。
- **CSS 语法问题**：`background-size`/`animation` 位于选择器块外被浏览器丢弃；`m()` 末尾多了一个 `}`。

### Changed

- **重构为 CSS 变量体系**：所有硬编码颜色替换为 `var(--mc-xxx, fallback)`，支持主题切换。
- **魔法面板样式统一**：`panelCSS` 中 13 处颜色值改为 `var(--mc-xxx, 原值)` 引用。
